...
%% Inset -> mean data of early and late lick onset

%Create empty figure
scr=get(0,'ScreenSize');
W=scr(3); H=scr(4);
position=[600,200,225,300];
default_font('Arial',24);
Fig = figure('Position',position,...
    'Units','inches','PaperUnits','inches','color','w');

color2 = [0.8500 0.3250 0.0980 ;0 0 1];
...
ylabel('Lick onset (s)');
set(gca, 'YTick', [0 0.4]) % 10 ticks
set(gca, 'XTick', [ 1 2 ]) % 10 ticks
set(gca, 'XTickLabel', {'early','late'}) % 10 ticks
set(gca,'linewidth',3)
set(gca, 'TickDir', 'out')
