...
W=scr(3); H=scr(4);
position=[100,200,300,225];
default_font('Arial',16);
Fig = figure('Position',position,...
    'PaperUnits','points','PaperPosition',position,'color','w');

time = -0.512:0.064:0.96; 
plot(time, Estimate_diff1(:,1),'-','color',[0 0 0],'LineWidth',2.5); hold on
plot(time, Estimate_diff1(:,2),'-','color',[109 110 113]/255,'LineWidth',2.5); hold on
plot(time, Estimate_diff1(:,3),'-','color',[147 149 152]/255,'LineWidth',2.5); hold on
yline(0,'--')
xlim([-0.5 1])
ylim([-11 10])
yticks([-10 0 10])
