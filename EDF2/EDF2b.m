...
for m =  1:length(mnames)
    ...
    lick1(m,:) =mean(opto_all.(mnames{m}).(snames{2}).lick.Go1_lick_rate); 
    lick1s(m,:) =mean(opto_all.(mnames{m}).(snames{2}).lick.Go1s_lick_rate);
    lick2(m,:) =mean(opto_all.(mnames{m}).(snames{2}).lick.NG1_lick_rate);
    lick2s(m,:) =mean(opto_all.(mnames{m}).(snames{2}).lick.NG1s_lick_rate); 
end
...
fill(xrange,yrange3,color2(3,:),'FaceAlpha',0.2,'EdgeColor','none');hold on
fill(xrange,yrange4,color2(4,:),'FaceAlpha',0.2,'EdgeColor','none');hold on
