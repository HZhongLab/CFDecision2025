...
for m =  1:length(mnames)
    ...
    lick1(m,:) =mean(opto_all.(mnames{m}).(snames{1}).lick.Go1_lick_rate); 
    lick1s(m,:) =mean(opto_all.(mnames{m}).(snames{1}).lick.Go1s_lick_rate);
    lick2(m,:) =mean(opto_all.(mnames{m}).(snames{1}).lick.NG1_lick_rate);
    lick2s(m,:) =mean(opto_all.(mnames{m}).(snames{1}).lick.NG1s_lick_rate); 
end
