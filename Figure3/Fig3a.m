...
%Categorize cells based on threshold (generalization)
High_2SD = find(diff_all1>threshold2);
Low_2SD = find(diff_all1<threshold1);
Outside_2SD = find(diff_all1<threshold1 | diff_all1>threshold2);
Within_2SD = find(diff_all1<threshold2 & diff_all1>threshold1);
...
for c = 1:length(diff_all1) 
    if diff_all1(c) < threshold1
        scatter(diff_all1(c),d_score_all1(c),60,"LineWidth",1.0,'MarkerFaceColor','none','MarkerEdgeColor','g'); hold on
    elseif diff_all1(c) > threshold2

        scatter(diff_all1(c),d_score_all1(c),60,"LineWidth",1.0,'MarkerFaceColor','none','MarkerEdgeColor','g'); hold on
    else
        scatter(diff_all1(c),d_score_all1(c),60,"LineWidth",1.0,'MarkerFaceColor','none','MarkerEdgeColor',[0.3 0.3 0.3]); hold on
    end
end
...
counts = [length(Outside_2SD)/length(d_score_all1) ,length(Within_2SD)/length(d_score_all1)];
